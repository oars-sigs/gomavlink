//autogenerated:yes
//nolint:revive,misspell,govet,lll,dupl,gocritic
package vkfly

import (
	"strconv"
	"fmt"
)
type MAV_ODID_OPERATOR_ID_TYPE uint64

const (
	// CAA (Civil Aviation Authority) registered operator ID.
	MAV_ODID_OPERATOR_ID_TYPE_CAA MAV_ODID_OPERATOR_ID_TYPE = 0
)

var value_to_label_MAV_ODID_OPERATOR_ID_TYPE = map[MAV_ODID_OPERATOR_ID_TYPE]string{
	MAV_ODID_OPERATOR_ID_TYPE_CAA: "MAV_ODID_OPERATOR_ID_TYPE_CAA",
}

var label_to_value_MAV_ODID_OPERATOR_ID_TYPE = map[string]MAV_ODID_OPERATOR_ID_TYPE{
	"MAV_ODID_OPERATOR_ID_TYPE_CAA": MAV_ODID_OPERATOR_ID_TYPE_CAA,
}

// MarshalText implements the encoding.TextMarshaler interface.
func (e MAV_ODID_OPERATOR_ID_TYPE) MarshalText() ([]byte, error) {
	if name, ok := value_to_label_MAV_ODID_OPERATOR_ID_TYPE[e]; ok {
		return []byte(name), nil
	}
	return []byte(strconv.Itoa(int(e))), nil
}

// UnmarshalText implements the encoding.TextUnmarshaler interface.
func (e *MAV_ODID_OPERATOR_ID_TYPE) UnmarshalText(text []byte) error {
	if value, ok := label_to_value_MAV_ODID_OPERATOR_ID_TYPE[string(text)]; ok {
	   *e = value
	} else if value, err := strconv.Atoi(string(text)); err == nil {
	   *e = MAV_ODID_OPERATOR_ID_TYPE(value)
	} else {
		return fmt.Errorf("invalid label '%s'", text)
	}
	return nil
}

// String implements the fmt.Stringer interface.
func (e MAV_ODID_OPERATOR_ID_TYPE) String() string {
	val, _ := e.MarshalText()
	return string(val)
}
